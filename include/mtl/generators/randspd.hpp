...
/// Construct a symmetric positive definite matrix with prescribed positive eigenvalues.
/// A = Q * diag(eigenvalues) * Q^T where Q is random orthogonal.
/// All eigenvalues must be strictly positive. eigenvalues.size() must equal n.
template <typename T = double>
auto randspd(std::size_t n, const std::vector<T>& eigenvalues) {
    assert(eigenvalues.size() == n);
    for (std::size_t i = 0; i < n; ++i)
        assert(eigenvalues[i] > T(0) && "randspd: all eigenvalues must be positive");
...
/// Construct an SPD matrix with eigenvalues determined by condition number and mode.
/// Uses the same sigma distribution as randsvd (modes 1-5).
/// Eigenvalues are in [1/kappa, 1], all positive by construction.
template <typename T = double>
auto randspd(std::size_t n, T kappa, int mode = 3) {
    auto eigenvalues = detail::make_sigma(n, kappa, mode);
    return randspd<T>(n, eigenvalues);
}
