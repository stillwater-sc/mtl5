...
/// Build unsymmetric convection-diffusion: A(i,i)=2, A(i,i-1)=-1-eps, A(i,i+1)=-1+eps
...
TEST_CASE("GMRES regression: 2D Laplacian, identity PC", "[regression][itl][gmres]") {
    auto k = GENERATE(50, 71);
    ...
    report("GMRES(30)", "none", "Laplacian-2D", n, iter.iterations(), rr, 1e-8);
    REQUIRE(err == 0);
    REQUIRE(rr < 1e-8);
}

TEST_CASE("GMRES regression: 1D convection-diffusion, ILU(0) PC", "[regression][itl][gmres]") {
    ...
    double rr = sparse_relative_residual(A, x, b);
    report("GMRES(30)", "ILU(0)", "ConvDiff-1D", n, iter.iterations(), rr, 1e-8);
    REQUIRE(err == 0);
    REQUIRE(rr < 1e-8);
}

TEST_CASE("GMRES regression: 2D Laplacian, Jacobi PC", "[regression][itl][gmres]") {
    ...
    double rr = sparse_relative_residual(A, x, b);
    report("GMRES(30)", "Jacobi", "Laplacian-2D", n, iter.iterations(), rr, 1e-8);
    REQUIRE(err == 0);
    REQUIRE(rr < 1e-8);
}
