...
TEST_CASE("IDR(s): dense 3x3 system", "[itl][krylov][idr_s]") {
    ...
    int err = itl::idr_s(A, x, b, pc, iter, 2);
    REQUIRE(err == 0);
...
TEST_CASE("IDR(s): sparse tridiagonal with s=4", "[itl][krylov][idr_s]") {
    ...
    int err = itl::idr_s(A, x, b, pc, iter, 4);
    REQUIRE(err == 0);
