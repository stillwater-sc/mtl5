...
// "gflops" column carries the solve THROUGHPUT (2*nnz / ns) so
// analyze_scaling's throughput ratio across thread counts == the wall-clock
// speedup. FactorFn(A) -> a factored solver exposing .solve(x, b).
...
static void bench_case(mtl::bench::reporter& rep, const std::string& op,
    ...
    auto t = mtl::bench::measure([&]{ num.solve(x, b); },
                                 op, label, n, /*flops=*/2.0 * nnz,
                                 /*warmup=*/3, /*iterations=*/25);
    rep.add(t);
    std::printf("  %-26s n=%-8zu nnz=%-9zu solve=%9.1f us  Mnnz/s=%8.1f  resid=%.2e\n",
                op.c_str(), n, A.nnz(), t.median_ns / 1e3,
                nnz / (t.median_ns / 1e3), rr);
}
...
static void run_suite(mtl::bench::reporter& rep, const std::string& label,
    ...
    for (std::size_t g : grid_sizes) {
        ...
        const std::string tag = "lap2d" + std::to_string(g);
        // SPD solvers on the 2-D Laplacian (many narrow levels).
        bench_case(rep, "chol " + tag, label, L2, [](const compressed2D<double>& A) {
            return fact::sparse_cholesky_numeric(A, fact::sparse_cholesky_symbolic(A, ord::amd{}));
        });
        bench_case(rep, "ldlt " + tag, label, L2, [](const compressed2D<double>& A) {
            return fact::sparse_ldlt_numeric(A, fact::sparse_ldlt_symbolic(A, ord::amd{}));
        });
        bench_case(rep, "snldlt " + tag, label, L2, [](const compressed2D<double>& A) {
            return fact::supernodal_ldlt_numeric(A, fact::supernodal_ldlt_symbolic(A, ord::amd{}));
        });
        // General LU on the same SPD matrix (treated as unsymmetric).
        bench_case(rep, "lu " + tag, label, L2, [](const compressed2D<double>& A) {
            return fact::sparse_lu_numeric(A, fact::sparse_lu_symbolic(A, ord::colamd{}));
        });
        bench_case(rep, "snlu " + tag, label, L2, [](const compressed2D<double>& A) {
            return fact::supernodal_lu_numeric(A, fact::supernodal_lu_symbolic_analyze(A, ord::colamd{}));
        });
    }

    // Arrow SPD (few wide levels): the optimistic solve-scaling case.
    {
        const std::size_t n = grid_sizes.empty() ? 20000 : grid_sizes.back() * grid_sizes.back() * 2;
        auto AR = arrow_spd(n);
        bench_case(rep, "chol arrow", label, AR, [](const compressed2D<double>& A) {
            return fact::sparse_cholesky_numeric(A, fact::sparse_cholesky_symbolic(A, ord::amd{}));
        });
    }

    // Unsymmetric banded: LU / supernodal LU on a genuinely non-SPD system.
    {
        const std::size_t n = grid_sizes.empty() ? 40000 : grid_sizes.back() * grid_sizes.back() * 4;
        auto UB = unsym_banded(n);
        bench_case(rep, "lu banded", label, UB, [](const compressed2D<double>& A) {
            return fact::sparse_lu_numeric(A, fact::sparse_lu_symbolic(A, ord::colamd{}));
        });
        bench_case(rep, "snlu banded", label, UB, [](const compressed2D<double>& A) {
            return fact::supernodal_lu_numeric(A, fact::supernodal_lu_symbolic_analyze(A, ord::colamd{}));
        });
    }
}
...
int main(int argc, char** argv) {
    ...
    for (const auto& f : files) {
        ...
        bench_case(rep, "lu " + nm, label, A, [](const compressed2D<double>& M) {
            return fact::sparse_lu_numeric(M, fact::sparse_lu_symbolic(M, ord::colamd{}));
        });
        try {
            bench_case(rep, "chol " + nm, label, A, [](const compressed2D<double>& M) {
