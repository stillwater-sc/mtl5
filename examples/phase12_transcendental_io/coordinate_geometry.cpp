// coordinate_geometry.cpp -- Trigonometric & Hyperbolic Geometry
//
...
int main() {
    ...
    // A set of points on a cardioid: r(theta) = 1 + cos(theta)
    ...
    // r(theta) = 1 + cos(theta) -- cardioid in polar coordinates
    ...
    std::cout << "Cardioid: r(theta) = 1 + cos(theta)\n\n";
    std::cout << std::setw(8) << "theta/pi"
              << std::setw(10) << "r"
              << std::setw(12) << "x"
              << std::setw(12) << "y\n";
    ...
    // Verify round-trip: r_recovered = sqrt(x^2 + y^2)
    ...
    // We can recover theta from R using acos and asin.

    const double angle = pi / 6.0;  // 30 degrees
    std::cout << "Rotation angle: pi/6 = " << std::fixed << std::setprecision(4)
              << angle << " rad (" << angle * 180.0 / pi << "deg)\n\n";

    mat::dense2D<double> Rot(2, 2);
    Rot(0, 0) = std::cos(angle);  Rot(0, 1) = -std::sin(angle);
    Rot(1, 0) = std::sin(angle);  Rot(1, 1) = std::cos(angle);

    std::cout << "R(pi/6) = [" << Rot(0,0) << ", " << Rot(0,1) << "]\n";
    ...
    std::cout << "  Expected: (cos pi/6, sin pi/6) = ("
    ...
    std::cout << "  acos(R_0_0) = " << recovered_acos << " rad (error: "
              << std::scientific << std::abs(recovered_acos - angle) << ")\n";
    std::cout << std::fixed << std::setprecision(4);
    std::cout << "  asin(R_1_0) = " << recovered_asin << " rad (error: "
    ...
    // Parabola approximation: y = a + x^2/(2a) (Taylor expansion of cosh)
    ...
    // Verify hyperbolic identity: cosh^2(x) - sinh^2(x) = 1
    ...
    std::cout << "\nIdentity check: max |cosh^2-sinh^2-1| = "
    ...
    std::cout << "Standard normal CDF: Phi(z) = 1/2[1 + erf(z/sqrt2)]\n\n";
    ...
    // Verify symmetry: Phi(z) + Phi(-z) = 1  <=>  erf(x) + erfc(x) = 1
    ...
    // The 68-95-99.7 rule
    std::cout << std::fixed << std::setprecision(4);
    double within_1sigma = cdf(6) - cdf(2);  // Phi(1) - Phi(-1)
    double within_2sigma = cdf(7) - cdf(1);  // Phi(2) - Phi(-2)
    double within_3sigma = cdf(8) - cdf(0);  // Phi(3) - Phi(-3)
    std::cout << "\nThe 68-95-99.7 rule:\n";
    ...
    // In electrical engineering, AC circuits use phasors: V = V_0 * e^(jphi)
    // real() and imag() extract in-phase and quadrature components.

    using cd = std::complex<double>;
    dense_vector<cd> phasors = {
        cd(5.0, 0.0),    // 5angle0deg  -- purely resistive
        cd(0.0, 3.0),    // 3angle90deg -- purely inductive
        cd(4.0, 3.0),    // 5angle36.87deg -- RL circuit
        cd(-2.0, 2.0),   // 2sqrt2angle135deg -- leading
    };
    ...
    std::cout << "3. The error function erf() builds the normal CDF: Phi(z) = 1/2[1+erf(z/sqrt2)].\n";
