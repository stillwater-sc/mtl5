...
| W3 | **Heavy data structures in hot path** | raw CSC int arrays, pre-sized/chunk-grown | block extraction ~~inserter~~ → **two-pass counting build into reused workspace (Phase 3a)**; B build + L/U growth still to do | [3 (#135)](native-klu-performance.md) | **Phase 3a (block extraction):** rajat30 4.5× → **4.2×** (32s → 30s); Poisson unchanged (single block, numeric-bound). Residual 4.2× ≈ 1.7× fill × **2.5× constant** lives in the **scalar GP-LU numeric gather/scatter** (profiling-driven micro-opt remaining) — extraction was not rajat30's bottleneck. Bigger win expected on many-block matrices. |
