...
| W4 | **Repeated symbolic + pivot search** | analyze / factor / **refactor** split | ~~no refactor~~ → **`sparse_lu_refactor` (Phase 4a)**: reuses pattern + pivot sequence, recomputes values only (no reach DFS, no pivot search) | [4 (#136)](native-klu-performance.md) | **Phase 4a:** refactor **2.1× faster than factor** (Poisson 200²: 123ms → 59ms). Correct (reproduces factor; solves same-pattern/new-values); ASan-clean. native-KLU analyze/factor/refactor split = 4b. |
