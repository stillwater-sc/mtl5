...
|---|---|---|
| Dense BLAS sweep, GEMM scaling | **2026-08-05** | Re-taken after the register-tile change (#382) |
...
- **native-fast is now the WORST multi-core scaler of the four**, at 6.33× on
  8 P-cores (79.2% efficiency) against BLIS 7.37×, MKL 7.21× and OpenBLAS 7.11×.

  > **Retracted:** this page previously reported native-fast as the *best*
  > scaler at 7.24× / 90.5%. That is no longer true, and the cause is #382
  > itself — see [what #382 did to scaling](#what-382-did-to-scaling). The
  > single-thread baseline rose 22%, so the same absolute multi-core throughput
  > now divides into a worse efficiency; and at N = 1024 the 8-thread number is
  > an outright **regression**, −7.4%. Tracked in #408.
...
|---:|---|---|---|---|
| 1 | 67.8 (1.00×) | 71.2 (1.00×) | 74.1 (1.00×) | 74.9 (1.00×) |
| 2 | 133.9 (1.97×) | 141.9 (1.99×) | 147.3 (1.99×) | 145.3 (1.94×) |
| 4 | 253.6 (3.74×) | 278.1 (3.91×) | 286.9 (3.87×) | 290.4 (3.88×) |
| 8 | 429.4 (**6.33×**) | **524.4 (7.37×)** | 526.8 (7.11×) | 540.4 (7.21×) |

### What #382 did to scaling

**Retracted: native-fast is no longer the best scaler; it is now the worst of the
four.** At 6.33× / **79.2% parallel efficiency** it sits behind BLIS (7.37×),
MKL (7.21×) and OpenBLAS (7.11×). The previous revision reported 7.24× / 90.5%.

This is a direct consequence of #382, and the per-thread deltas show it clearly:
...
The gain decays monotonically with thread count, and at N = 1024 with 8 threads
it goes **negative** — an absolute regression, 388.9 → 360.2 GFLOP/s. So two
things are true at once: the serial kernel got much faster (which is why the #82
gate now reads 97.4%), and the parallel efficiency got worse, partly because the
same speedup is being measured against a 22% higher single-thread baseline and
partly because at the smaller size the multi-core result actually fell.

A plausible mechanism is that the larger register tile raises each thread's
working set, so eight threads contend for cache and bandwidth in a way one
thread does not — which would also explain why the regression appears at
N = 1024 before N = 2048. **This page does not claim that**; it was not tested,
and the one hypothesis that *was* tested on this page (the MKL frequency story
above) turned out to be wrong. Tracked in **#408**.
...
thread counts (75.5% at T=1, 76.4% at T=8). It is no longer flat: native-fast is
now **91.5% of OpenBLAS at one thread but 81.5% at eight**. The differentiator
has moved from the single-core kernel to the parallel path — the reverse of what
